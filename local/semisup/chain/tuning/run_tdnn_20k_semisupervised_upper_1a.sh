...
dir=$exp_root/chain${chain_affix}/${tdnn_semi}_all_correct
...
if [ $stage -le 1 ]; then
  ...
  $data_root/${unsupervised_set_perturbed} $min_seg_len $data_root/${unsupervised_set_perturbed}_comb
...
#  ln -sf ../final.mdl $sup_chain_dir/decode_${unsupervised_set_perturbed}_big/final.mdl
  steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd --num-threads 5" --generate-ali-from-lats true \
                            $data_root/${unsupervised_set_perturbed}_comb data/lang $gmm_dir  \
                            $sup_chain_dir/align_${unsupervised_set_perturbed}_big
fi
...
# used as frame-weights in lattice-based training
  ...
  #  $data_root/${unsupervised_set_perturbed}_hires_comb \
  #  $sup_chain_dir/align_${unsupervised_set_perturbed}_big \
  #  $sup_chain_dir/best_path_align_${unsupervised_set_perturbed}_big
#fi
...
if [ $stage -le 10 ]; then
  steps/nnet3/chain/make_weighted_den_fst.sh --num-repeats $lm_weights --cmd "$train_cmd" \
    ${sup_tree_dir} ${sup_chain_dir}/align_${unsupervised_set_perturbed}_big \
    $dir
fi
...
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 13 ]; then
    ...
    steps/nnet3/chain/get_egs.sh \
      ...
      #--deriv-weights-scp $sup_chain_dir/best_path_align_${unsupervised_set_perturbed}_big/weights.scp \
  fi
fi
