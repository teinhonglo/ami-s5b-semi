...
if [ -d data/$mic/semisup/$train_sup_dir ]; then
    ...
    utils/subset_data_dir.sh --spk-list <(utils/filter_scp.pl --exclude data/$mic/semisup/$train_sup_dir/spk2utt \
                                         data/$mic/train/spk2utt) data/$mic/train data/$mic/semisup/$train_unsup_dir
fi							 

if [ $stage -le 13 ]; then
   ali_opt=
  [ "$mic" != "ihm" ] && ali_opt="--use-ihm-ali true"
  local/semisup/run_20k.sh --mic $mic --train_sup_dir $train_sup_dir \
                                      --train_unsup_dir $train_unsup_dir \
                                      --data-root data/ihm/semisup \
                                      --exp-root exp/ihm/semisup_20k \
                                      --stage $semi_stage
  exit 0;
fi
...
if [ $stage -le 14 ]; then
  ali_opt=
  [ "$mic" != "ihm" ] && ali_opt="--use-ihm-ali true"
  local/adapt/run_20k.sh --mic $mic \
                         --train_sup_dir $train_sup_dir \
                         --train_unsup_dir $train_unsup_dir \
                         --semi_dir semisup --stage $semi_stage
  exit 0;
fi
...
if [ $stage -le 15 ]; then
  ...
  [ "$mic" != "ihm" ] && ali_opt="--use-ihm-ali true"
  # Create supervised and unsupervised data subsets
  local/fusion_ts/run_20k.sh --mic $mic \
                             --train_sup_dir $train_sup_dir \
                             --train_unsup_dir $train_unsup_dir \
                             --semi_dir semisup --stage $semi_stage
  exit 0;						   
